...
score is a real state and neither answer may be assumed. Two authoring mistakes are
now refused loudly rather than delivered silently: a `stimulusRef` that resolves to
nothing, and a correctness gate over an item without unlimited Tries — a learner
who spent a finite budget could never release playback again.
...
`SectionControllerHandle`, and three controller events —
`timed-media-cue-changed`, `timed-media-policy-degraded`, `timed-media-invalid`.
A host switching exhaustively over `SectionControllerEvent["type"]` with no
`default` gains three variants; everything else is optional and absent reads as
`null`.
...
Two fixes fall out of building this, and both affect sections that carry no timed
media at all. The engine's controller subscription now isolates its event handler,
so a diagnostic that throws can no longer abort the listener before the composition
republish — that failure stopped *every* controller event in a section, formative
Tries included, behind nothing but a console warning. And the section content
service now records how a `stimulusRef` resolves to a rendered renderable, rather
than leaving each caller to re-derive it.
