...
test.describe("A11y component fixtures", () => {
	for (const fixture of A11Y_FIXTURES) {
		test(`${fixture.id} should not have WCAG 2.2 Level AA violations`, async ({
			page,
		}) => {
			await page.goto(`/a11y-components/${fixture.id}`);
			await page.waitForLoadState("networkidle");

			const root = page.locator('[data-testid="a11y-fixture-root"]').first();
			...
			if (results.violations.length > 0) {
				console.log(
					`A11y violations for ${fixture.id}:`,
