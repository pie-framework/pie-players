...
const demo: DemoInfo = {
	...
	item: {
		...
		config: {
			id: "",
			markup:
				'<graphing-solution-set id="inequality-solid-line"></graphing-solution-set>',
			elements: {
			...
			models: [
				{
					...
					prompt:
						"Graph the solution set for the inequality: <math>y \\leq -x + 4</math>",
					promptEnabled: true,
					title: "Linear Inequality with Solid Boundary",
					rationale:
						"The inequality y ≤ -x + 4 requires a solid line (since it includes ≤) at y = -x + 4, with the region below and including the line shaded as the solution set.",
					rationaleEnabled: true,
					teacherInstructions:
						"Students should draw a solid line through points like (0, 4) and (4, 0), then select the region below the line as the solution set.",
