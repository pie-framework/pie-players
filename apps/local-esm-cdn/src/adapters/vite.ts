...
export function createVitePlugin(config: Partial<LocalEsmCdnConfig>): Plugin {
  ...
  return {
    ...
    resolveId(id) {
      ...
      // Let Vite handle @pie-framework packages normally (they're workspace deps)
      if (
    ...
    configureServer(serverInstance) {
      ...
      if (!config.pieElementsNgRoot && !config.piePlayersRoot) {
        console.warn('[vite-plugin-local-esm-cdn] No repo roots configured, skipping file watching');
        return;
      }
      ...
      if (config.pieElementsNgRoot) {
        watchPaths.push(
          path.join(config.pieElementsNgRoot, 'packages/elements-react/*/dist/**'),
          path.join(config.pieElementsNgRoot, 'packages/elements-svelte/*/dist/**'),
  ...
  function getPackageNameFromDistPath(filePath: string): string | null {
    ...
    // pie-players/packages/<package-name>/dist -> @pie-framework/pie-<package-name>
    ...
    if (playersMatch) {
      ...
      return `@pie-framework/pie-${pkgName}`;
    }
