<script lang="ts">
	...
	const inputId = `section-demos-element-version-input-${Math.random().toString(36).slice(2, 11)}`;
	const listboxId = `section-demos-element-version-list-${Math.random().toString(36).slice(2, 11)}`;
	...
	function updateDropdownPosition() {
		...
		const rect = inputWrapperElement.getBoundingClientRect();
...
<div class="relative inline-flex items-center {compact ? 'w-auto' : 'w-full'}" bind:this={containerElement}>
	...
	{/if}
	<div class="relative {compact ? 'w-44' : 'w-full'}" bind:this={inputWrapperElement}>
		<input
			id={inputId}
			type="text"
	...
	{#if dropdownVisible}
		<div
			id={listboxId}
			class="pie-section-demos-version-selector__listbox bg-base-100 border border-base-300 rounded-md shadow-xl overflow-y-auto"
...
<style>
	.pie-section-demos-version-selector__listbox {
		position: fixed;
		max-height: 15rem;
		z-index: 2147483000;
	}
