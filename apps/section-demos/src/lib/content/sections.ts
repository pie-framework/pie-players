...
export const sectionDemos: Record<string, SectionDemoInfo> = {
	...
	'tts-ssml': {
		id: 'tts-ssml',
		name: 'TTS with SSML',
		description: 'Three items with passage - demonstrates SSML extraction, AWS SSML tags, and multi-level TTS',
		section: demo4Section
	}
};

export function getSectionDemoById(id: string | undefined): SectionDemoInfo | null {
	if (!id) return null;
