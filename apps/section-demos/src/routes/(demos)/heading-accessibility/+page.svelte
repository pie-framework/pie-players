<script lang="ts">
	...
	// baseHeadingLevel: rewrite data-heading paragraphs to real heading elements.
	// includeSrHeading: toggle the visually-hidden SR heading at the top of the player.
	let baseHeadingLevel = $state<1 | 2 | 3 | 4 | 5 | 6>(2);
	let includeSrHeading = $state(true);
	...
	$effect(() => {
		if (!sessionDebuggerElement) return;
		return wireCloseListener(sessionDebuggerElement, () => { showSessionPanel = false; });
	});
	$effect(() => {
		if (!pnpDebuggerElement) return;
		return wireCloseListener(pnpDebuggerElement, () => { showPnpPanel = false; });
	});
	$effect(() => {
		if (!eventDebuggerElement) return;
		return wireCloseListener(eventDebuggerElement, () => { showEventPanel = false; });
	});
	$effect(() => {
		if (!instrumentationDebuggerElement) return;
		return wireCloseListener(instrumentationDebuggerElement, () => { showInstrumentationPanel = false; });
	});

	$effect(() => {
		if (!browser) return;
		const triggerSessionPanelRefresh = () => {
			queueMicrotask(() => { sessionDebuggerElement?.refreshFromHost?.(); });
		};
		...
		return () => {
			document.removeEventListener('item-session-changed', triggerSessionPanelRefresh as EventListener, true);
			document.removeEventListener('session-changed', triggerSessionPanelRefresh as EventListener, true);
			document.removeEventListener('item-session-changed', persistSectionSession as EventListener, true);
	...
	async function resetSessions() {
		...
		if (browser) {
			...
			for (let index = 0; index < window.localStorage.length; index += 1) {
				...
				if (!key) continue;
				if (key.startsWith(DEMO_PERSISTENCE_STORAGE_PREFIX)) keysToRemove.push(key);
			}
...
<svelte:head>
	<title>{data.demo?.name || 'Heading Accessibility Demo'}</title>
</svelte:head>

<!-- Demo-specific controls for baseHeadingLevel and includeSrHeading -->
<div class="a11y-controls" data-testid="heading-a11y-controls">
	<label class="a11y-control-label">
		<span>baseHeadingLevel</span>
		<select
			data-testid="base-heading-level-select"
			bind:value={baseHeadingLevel}
		>
	...
	<label class="a11y-control-label">
		<input
			...
			bind:checked={includeSrHeading}
		/>
		<span>includeSrHeading</span>
	</label>
</div>
