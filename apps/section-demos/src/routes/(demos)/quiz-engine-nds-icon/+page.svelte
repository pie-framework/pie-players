<script lang="ts">
	...
	// Reads the demo app's Desmos auth endpoint so the calculator tool (the
	// tool that renders as an <nds-icon-button> in the item toolbar) can boot.
	...
	// Same CE-first setup as Single Question, but with `calculator` placed on
	// the item toolbar so the NDS icon buttons (toolbar + floating shell) show
	// alongside the NDS inline-TTS trigger. The opt-in itself is the
	// `nds-icons` attribute on the player element below.
...
<svelte:head>
	<title>{data.demo?.name || 'Demo'} - NDS Icons</title>
</svelte:head>
