<script lang="ts">
	...
	async function bootstrapSessionDemoDb(reset = true) {
		const response = await sessionDbApi.bootstrap({
			assessmentId: DEMO_ASSESSMENT_ID,
			attemptId,
	...
	async function loadSnapshotFromDb(sectionId: string) {
		return await sessionDbApi.loadSnapshot({
			assessmentId: DEMO_ASSESSMENT_ID,
			sectionId,
			attemptId
		});
	...
	async function saveSnapshotToDb(sectionId: string, snapshot: any) {
		await sessionDbApi.saveSnapshot({
			assessmentId: DEMO_ASSESSMENT_ID,
			sectionId,
			attemptId,
			snapshot
	...
	async function deleteSnapshotFromDb(sectionId: string) {
		await sessionDbApi.deleteSnapshot({
			assessmentId: DEMO_ASSESSMENT_ID,
			sectionId,
			attemptId
		});
