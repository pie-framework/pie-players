...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		if (images.length === 0) {
			console.log('[picture-dictionary] No mock images, returning placeholder for:', keyword);
			images = [
				`https://via.placeholder.com/200x200/cccccc/666666?text=${encodeURIComponent(keyword)}`
			];
		}

		console.log('[picture-dictionary] Returning %d images for keyword: %s', images.length, keyword);
		...
		console.error(`[picture-dictionary] Failed to load pictures for keyword "${keyword}":`, e);
