...
- **SHARED-01 · Medium · sast/arch** — Dynamic PIE element loader.
  - Files (at scan time, 2026-04-20): `packages/players-shared/src/pie/esm-loader.ts` (dynamic `import(specifier)` at line ~340 into `defineCustomElementSafely(actualTag, class extends ElementClass {}, …)` at lines 404-408); `packages/players-shared/src/pie/iife-loader.ts` line 385 (`script.src = url`); [packages/players-shared/src/pie/initialization.ts](packages/players-shared/src/pie/initialization.ts) lines 509, 560 (`script.src = url`, `script.src = bundleUrl`). URL assembly was at lines 159 / 162 / 167 / 169 / 177 / 179 of `esm-loader.ts`.
  - Files (post PIE-501 element-loader refactor — superseding the scan-time refs above): [packages/players-shared/src/loaders/ElementLoader.ts](../packages/players-shared/src/loaders/ElementLoader.ts) (consolidated primitive), [packages/players-shared/src/loaders/iife-adapter.ts](../packages/players-shared/src/loaders/iife-adapter.ts) (`script.src = url` at line ~433), [packages/players-shared/src/loaders/esm-adapter.ts](../packages/players-shared/src/loaders/esm-adapter.ts) (dynamic `import(specifier)`; `cdnBaseUrl` URL assembly at lines ~298-396), [packages/players-shared/src/pie/initialization.ts](../packages/players-shared/src/pie/initialization.ts) (`script.src = url` / `script.src = bundleUrl` at lines ~574, ~625).
  - Evidence: loaders accept a `cdnBaseUrl` (or `bundleUrl`) string and dynamically import/inject scripts from it. Package names come from the item config's element registry.
  ...
  - Status: trust-boundary finding stands; only the file references were retargeted after the loader consolidation.
