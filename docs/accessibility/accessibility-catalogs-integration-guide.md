...
PIE items reference catalogs using the `data-catalog-idref` attribute (either manually authored or auto-generated by SSML extraction):

```html
<!-- Prompt with catalog reference -->
<pie-prompt>
  <div data-catalog-idref="prompt-001">
...
<pie-choices>
  <pie-choice value="A" data-catalog-idref="choice-001-A">
    Plants need sunlight to grow
  </pie-choice>
  <pie-choice value="B" data-catalog-idref="choice-001-B">
...
const item = {
  ...
  markup: `
    <pie-stimulus>
      ...
      <div data-catalog-idref="shared-passage-001">...</div>
    </pie-stimulus>
    <pie-prompt>
      <!-- References item-level catalog -->
      <div data-catalog-idref="prompt-photo-001">...</div>
    </pie-prompt>
  `
};
```
...
export class TTSService implements ITTSService {
  ...
  /**
   * Optional custom selection method with catalog support.
   *
   * The built-in annotation toolbar read-aloud path intentionally speaks the
   * selected visible range and bypasses catalogs. Use this pattern only for a
   * custom selection tool that should prefer authored alternatives.
   */
  async speakSelectionWithCatalogs(
    range: Range,
...
export class MultipleChoiceController {
  ...
  private extractCatalogId(html: string): string | null {
    const match = html.match(/data-catalog-idref=["']([^"']+)["']/);
...
export const AccessiblePrompt: React.FC<{ content: string; catalogId?: string }> = ({
  ...
  return (
    <div data-catalog-idref={catalogId} className="accessible-prompt">
