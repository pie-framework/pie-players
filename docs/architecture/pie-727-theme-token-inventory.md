...
`--pie-section-player-tab-zoom-comp` is retained in the registry as `deprecated`.
It no longer affects layout: the width-ratio estimator mistook constrained hosts
for browser zoom and shrank controls below usable sizes. Tabs now follow native
browser scaling and the existing responsive layout.
