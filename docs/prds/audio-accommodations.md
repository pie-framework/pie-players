...
Outside this repo, unverified from here: the content half — [Sequencing](#sequencing) step 1 plus the additive per-program autoplay path — was written and behaviourally inert by design, sitting **uncommitted** on `pie-api-aws` `feat/PIE-881-asl-video-import` as of 2026-08-08. PIE-881 itself — the ASL signing work tracked on that branch — resolved Done on 2026-08-10 (commits `621f351d`, `43d28521`), but that ticket's scope is the sign-language transform, not this transcript/autoplay content half; whether the latter has since landed and the affected profiles grant `transcript` is still unverified from `pie-players`. Until confirmed, the legacy class is still what delivers the accommodation, and step 3 must not run.
...
The additive path also declines to inherit two defects in the legacy helper, both of which silently produce autoplay OFF on items that should have it. They are left in place rather than fixed, because fixing them moves live content underneath itself in the same breath as an open product question — and the measurement below says which content.
...
Counted against prod on 2026-08-17. The store holds one document per item version — 39,044 versions, 35,754 of them carrying a Star collection tag — so the population is stated by distinct `baseId`: **112 items change answer**, across 467 versions. The array-order defect never fired; no item carries more than one value in the plain tag.

- **99 items, all `CATITMST_PREVIEW`, all with prompt audio.** Star Early Literacy content whose plain tag is `STARCOL_STAREARLYLITERACYLSY` — no subject substring, hence OFF — while its multi tag adds `STARCOL_STARREADINGK12`. A third mechanism neither bullet above describes, and the population that matters: reading both tags turns autoplay ON for Early Literacy content in the SR PK12 collection, which is the collection the open product question is about.
- **13 items reachable only through the multi tag** — the documented defect, and nearly inert. Eleven are `CATITMST_ARCHIVED`, one `CATITMST_PENDING`, one `CATITMST_OPERATIONAL`; only three carry prompt audio, and the operational one is not among them. They are genuine bank content, not fixtures — real skill titles, `APPS_SR`/`APPS_SM`, `SMPLITM_FALSE` — imported in two small scripted batches.

So no operational item with prompt audio changes answer today. `CATITMST_PREVIEW` is the largest status in the store (13,435 versions against 2,051 operational), so whether the 99 reach a learner is a content-ops question, not a dormancy claim.

Direction is one-way — false→true only, so nothing that autoplays today stops — and nothing changes until affected items are re-imported. Five mappers (clozetext, association, graphplotting, numberlineplot, clozedropdown) assign both flags with no `hasStarAudio` guard, so a re-import also writes `true` onto affected items with no audio: inert at delivery, visible in a model diff.

**The policy layer cannot address 195 items as authored.** They record their collections as a single comma-joined array entry — `STARCOL_STARREADINGFAST,STARCOL_STARREADINGFAST,STARCOL_STARREADINGFAST` and `STARCOL_STARREADINGK12,STARCOL_STARREADINGLSY` are the common shapes. The substring subject rule reads through those; `collectStarCollectionTokens` yields the whole joined string as one token, and `byCollection` matches tokens exactly, so no per-program map can name them until the collector splits on comma.

Both rules now read the same tokens and differ only in what they do with them — substring subject test versus exact lookup — so a policy can name any collection the legacy rule can see.
