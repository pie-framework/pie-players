...
- **Pre-recorded audio as a spoken alternate.** *(See also [Audio Accommodations](./audio-accommodations.md), which picks up the audio thread — transcript gating and autoplay control — and leaves the card form here.)* Noticed 2026-08-08 while checking the shipped card shape against QTI 3's catalog examples. In QTI, `spoken` is not SSML-only: a card may hold `qti-file-href` with a `mime-type`, meaning a recorded audio file for that node, and `qti-card-entry@data-reading-type` distinguishes reading modes such as `computer-read-aloud`. PIE's `spoken` card is a string, so it has no way to say "play this file for this node" — independent of QTI, that is a capability PIE lacks, and human-recorded audio is a real accommodation some programs prefer over synthesized speech. The either/or card shape already leaves the right room: a `spoken` card carrying a `payload` with a `MediaAssetRef` of `kind: "audio"`, resolved the way signing is. Open only in the sense of *whether anyone wants it* — no customer has asked, so nothing is built. Worth knowing the shape costs nothing to add later.
