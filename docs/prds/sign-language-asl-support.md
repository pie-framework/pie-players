...
- **Pre-recorded audio as a spoken alternate.** *(See also [Audio Accommodations](./audio-accommodations.md), which picks up the audio thread — transcript gating and autoplay control — and leaves the card form here.)* Noticed 2026-08-08 while checking the shipped card shape against QTI 3's catalog examples. In QTI, `spoken` is not SSML-only: a card may hold `qti-file-href` with a `mime-type`, meaning a recorded audio file for that node. PIE's `spoken` card is a string, so it has no way to say "play this file for this node" — independent of QTI, that is a capability PIE lacks, and human-recorded audio is a real accommodation some programs prefer over synthesized speech. The either/or card shape already leaves the right room: a `spoken` card carrying a `payload` with a `MediaAssetRef` of `kind: "audio"`, resolved the way signing is. Not built, and the type is deliberately not declared ahead of a consumer: `TTSService` is a synthesis pipeline throughout — `hasSpokenAlternate` tests `content !== undefined`, and word highlighting is driven by provider boundary events that a recording does not emit — so a declared payload variant nothing honours would render nothing while reporting absent, which is the same failure the withdrawn `signLanguage` alias caused. The real prerequisite is a decision about timing marks for highlighting, not a type.
- **`CatalogType` accepts anything, and nothing validates it.** Corrected 2026-08-08: the union ends in `| string` ("Support custom types"), so it is open by construction. Vendor-extension tokens — QTI reserves `ext:`-prefixed `support` values, and its own example pairs `ext:custom-text-to-speech-pronunciation` with a `spoken` card on the same node — are therefore already representable, stored, and resolvable by `getAlternative({ type: "ext:..." })`; they simply have no consumer. The real question is the other side of that openness: the named literals are documentation only, so `"spokn"` is an equally valid `CatalogType` and a mistyped card silently never resolves. Open: whether the type should narrow to known tokens plus an explicit `` `ext:${string}` `` escape hatch, which would catch typos while keeping extensions expressible.
