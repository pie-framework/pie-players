...
| `a11y-008` | Post-navigation focus placement in section player | [2.4.3 Focus Order](https://www.w3.org/WAI/WCAG22/Understanding/focus-order), [2.4.7 Focus Visible](https://www.w3.org/WAI/WCAG22/Understanding/focus-visible), [2.4.11 Focus Not Obscured](https://www.w3.org/WAI/WCAG22/Understanding/focus-not-obscured-minimum) | High | Default `SectionPlayerFocusPolicy.autoFocus` (`start-of-content` / `current-item` / `none`) moves focus to passage or item card after navigation; item cards then call `pie-item-player.focusFirst()` so focus can enter the first control inside **open** shadow roots. | Closed shadow PIE elements, first-interactive heuristics across all element types, and sticky-toolbar obscuring still need AT / UX review. | Manual AT pass (`a11y-005`); confirm live-region assertive vs polite with a11y; extend coverage if new element shells ship. | `packages/section-player/tests/section-player-navigation-contract.spec.ts` | Partially addressed — evidence + edge cases still deferred |
