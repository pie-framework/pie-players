...
async function evaluateCalculator() {
  ...
  const scientificButton = page.locator('button').filter({ hasText: 'Scientific' });
  await scientificButton.click();
  await page.waitForTimeout(500);

  // Helper function to test a calculation
  async function testCalculation(expression: string, expectedPattern: RegExp | string, description: string) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    // Get computed styles
    const calcButtons = buttons.filter(b =>
      b.textContent && /^[0-9+\-×÷=AC]$/.test(b.textContent.trim())
    );
    ...
    // Get display styling
    const display = document.querySelector('input[readonly]') as HTMLInputElement;
  ...
  metrics.buttonSizes.forEach((btn: any) => {
    console.log(`  ${btn.text}: ${btn.width.toFixed(1)}px × ${btn.height.toFixed(1)}px`);
  });
  console.log(`\nDisplay Font: ${metrics.displayFontSize} (weight: ${metrics.displayFontWeight})`);
  console.log(`Calculator Size: ${metrics.calculatorWidth?.toFixed(0)}px × ${metrics.calculatorHeight?.toFixed(0)}px`);
  ...
  const a11y = await page.evaluate(() => {
    const buttons = Array.from(document.querySelectorAll('button'));
    const calcButtons = buttons.filter(b =>
      b.closest('[role="grid"]') || b.getAttribute('aria-label')
    );

    const hasAriaLabels = calcButtons.filter(b => b.getAttribute('aria-label')).length;
  ...
  console.log(`ARIA Labels: ${a11y.buttonsWithAriaLabels}/${a11y.totalCalculatorButtons} buttons`);
