...
Returns: List of generated image file paths and metadata.`,
            inputSchema: {
              ...
              properties: {
                ...
                outputDir: {
                  type: 'string',
                  description: 'Directory to save images (default: platform-specific)',
                },
                outputPrefix: {
                  type: 'string',
                  description: 'Prefix for output filenames (default: nanobanana)',
                  default: 'nanobanana',
                },
              },
              required: ['prompt'],
...
Returns: Path to the edited image.`,
            inputSchema: {
              ...
              properties: {
                ...
                outputDir: {
                  type: 'string',
                  description: 'Directory to save edited image (default: platform-specific)',
                },
                outputPrefix: {
                  type: 'string',
...
Returns: Path to the newly edited image.`,
            inputSchema: {
              ...
              properties: {
                ...
                outputPrefix: {
                  type: 'string',
                  description: 'Prefix for output filename (default: continued)',
                  default: 'continued',
                },
              },
              required: ['prompt'],
...
Returns: Confirmation message.`,
            inputSchema: {
              ...
              properties: {
                apiKey: {
                  type: 'string',
                  description: 'Your Google API key from https://aistudio.google.com/',
                },
              },
              required: ['apiKey'],
            },
...
Returns comprehensive information about Pro and Fast variants, use cases, and recommendations.`,
      ...
      try {
        ...
        switch (name) {
          case 'nanobanana_generate':
            return await this.handleGenerateImage(args as unknown as GenerateImageOptions);

          case 'nanobanana_edit':
            return await this.handleEditImage(args as unknown as EditImageOptions);

          case 'nanobanana_continue_editing':
            return await this.handleContinueEditing(args as unknown as ContinueEditingOptions);
  ...
  private async handleGenerateImage(args: GenerateImageOptions) {
    ...
    const modelName = result.model === 'gemini-3-pro-image-preview' ? 'Nano Banana Pro 🍌' : 'Nano Banana 🍌';
...
Edited with ${modelName} - Gemini's native image generation AI.`,
  ...
  private async handleGetLastImage() {
    ...
    const modelName = lastImage.model === 'gemini-3-pro-image-preview' ? 'Nano Banana Pro' : 'Nano Banana';
...
Nano Banana is part of Gemini's conversational AI, making image generation as easy as describing what you want to see!`,
  ...
  async start(): Promise<void> {
    try {
      ...
      const apiKey = configManager.getApiKey();
      if (!apiKey) {
        logger.warn('No API key configured. Use nanobanana_configure tool to set it up.');
      } else {
