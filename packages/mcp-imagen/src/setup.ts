...
export async function runSetup(): Promise<void> {
  ...
  console.log('This wizard will help you configure the Nano Banana (Gemini Image) MCP server.\n');
  ...
  try {
    ...
    const apiKey = await rl.question('Enter your Google API key (or press Enter to keep current): ');
    ...
    console.log('  1. gemini-3-pro-image-preview (Nano Banana Pro - recommended)');
    ...
    const modelChoice = await rl.question('Select default model (1-3, or press Enter for Standard): ');
    ...
    const finalOutputDir = outputDir.trim() || existingConfig.defaultOutputDir || process.cwd();
