...
describe("validateExternalStyleUrl", () => {
	...
	test("rejects javascript: URLs", () => {
		const result = validateExternalStyleUrl("javascript:alert(1)", {
			baseUrl: BASE,
		});
	...
	test("rejects data: URLs", () => {
		const result = validateExternalStyleUrl("data:text/css,body{}", {
			baseUrl: BASE,
		});
		expect(result.ok).toBe(false);
		if (!result.ok) {
	...
	test("rejects cross-origin URL when not on the allow-list", () => {
		const result = validateExternalStyleUrl("https://evil.example/style.css", {
			baseUrl: BASE,
