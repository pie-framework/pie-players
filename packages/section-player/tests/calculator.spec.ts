...
test('calculator functionality', async ({ page }) => {
  ...
  for (const link of demoLinks) {
    ...
    if (text && (text.includes('demo') || text.includes('Demo') || text.includes('question') || text.includes('Question'))) {
  ...
  if (!demoFound && demoLinks.length > 0) {
    console.log('No specific demo link found, clicking first available element');
  ...
  if (!calculatorButton) {
    ...
    console.log(`Searching through ${allButtons.length} buttons for calculator`);
    ...
    for (const button of allButtons) {
      ...
      if (html.toLowerCase().includes('calculator') || 
  ...
  const calculatorVisible = await page.locator('[class*="calculator"], [data-testid*="calculator"]').count() > 0;
  ...
  expect(consoleErrors.filter(err => err.includes('Calculator')).length, 'Should not have calculator-related errors').toBe(0);
});
