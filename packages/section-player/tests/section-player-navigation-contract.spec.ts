...
test.describe("section player navigation contract", () => {
	...
	test("navigation does not move focus to PIE content containers", async ({
		page,
	}) => {
		...
		const focusStayedOutsideContainers = await page.evaluate(async () => {
			...
			return (
				...
				!active?.closest?.(
					"pie-section-player-passage-card, pie-section-player-item-card, .pie-section-player-passages-pane, .pie-section-player-content-card[data-section-item-card]",
				)
			);
		});

		expect(focusStayedOutsideContainers).toBe(true);
	});

	test("splitpane does not expose passage or question containers as tab stops", async ({
		page,
	}) => {
		await page.goto(SPLIT_DEMO, { waitUntil: "networkidle" });
		await expect(
