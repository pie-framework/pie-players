...
<script lang="ts">
	...
	function isSeekControlFocused(): boolean {
		if (!toolbarEl) return false;
		const root = toolbarEl.getRootNode();
		if (!(root instanceof ShadowRoot)) return false;
		const active = root.activeElement as HTMLElement | null;
		if (!active || !toolbarEl.contains(active)) return false;
		return active.matches(SEEK_BUTTON_SELECTOR);
	}

	// Rewind / fast-forward go inactive the moment reading ends, and a disabled
	// element cannot keep focus — the browser would drop it to <body>. Hand focus
	// to Stop, which stays active, falling back to the play/pause trigger if the
	// panel has closed in the meantime.
	function moveFocusOffDisabledSeekControl(): void {
		const stop = toolbarEl?.querySelector<HTMLButtonElement>(STOP_BUTTON_SELECTOR);
	...
	const SEEK_BUTTON_SELECTOR = '[data-pie-tts-seek]';
	const STOP_BUTTON_SELECTOR = '[data-pie-tts-stop]';
		...
		{#snippet controlsPanel()}
			{#if controlsVisible}
				...
				>
					...
					<!-- Stop stays active for as long as the panel is open — including
					     after reading finishes on its own, when rewind / fast-forward go
					     inactive. It always has work to do (halt playback, dismiss the
					     controls), and staying enabled means it never loses focus it
					     already had. -->
					<button
						type="button"
						data-pie-tts-control
