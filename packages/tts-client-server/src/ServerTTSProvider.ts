...
class ServerTTSProviderImpl implements ITTSProviderImplementation {
   ...
   * Call server API to synthesize speech
   */
  private async synthesizeSpeech(text: string): Promise<{ audioUrl: string; wordTimings: WordTiming[] }> {
   ...
   */
  private parseSpeechMarks(marks: SynthesizeAPIResponse['speechMarks']): WordTiming[] {
    return marks
  ...
  private startWordHighlighting(): void {
    ...
    if (!this.currentAudio || !this.onWordBoundary || this.wordTimings.length === 0) {
    ...
    console.log('[ServerTTSProvider] Starting word highlighting with', this.wordTimings.length, 'word timings');
    console.log('[ServerTTSProvider] Playback rate:', this.currentAudio.playbackRate);
    console.log('[ServerTTSProvider] First 3 timings:', this.wordTimings.slice(0, 3));
      ...
      for (let i = 0; i < this.wordTimings.length; i++) {
        ...
        if (currentTime >= timing.time && i > lastWordIndex) {
          ...
          if (this.onWordBoundary) {
            console.log('[ServerTTSProvider] Highlighting word at charIndex:', timing.charIndex, 'length:', timing.length, 'time:', timing.time, 'currentTime:', currentTime);
