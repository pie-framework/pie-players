 ...
 */
export function estimateSpeechMarks(text: string, avgWordsPerMinute = 150): SpeechMark[] {
 ...
 */
export function adjustSpeechMarksForRate(marks: SpeechMark[], rate: number): SpeechMark[] {
  if (rate === 1.0) {
 ...
 */
export function getSpeechMarkAtTime(marks: SpeechMark[], time: number): SpeechMark | null {
  ...
  while (left <= right) {
    ...
    // Track closest mark
    if (!closest || Math.abs(mark.time - time) < Math.abs(closest.time - time)) {
      closest = mark;
    }
