 ...
 * publishes every publishable workspace package together, so a single missing trusted
 * publisher shows up as a partial release. This distinguishes the two failure modes
 * explicitly — published-without-provenance versus not-published-at-all.
 *
 * `@pie-players/pie-preloaded-player` is deliberately out of scope. It is generated at
 * build time from configs/preloaded-player/, published by its own workflow, and versioned
 * as `{loaderVersion}-{configHash}.{iteration}` rather than on the fixed release version,
 * so there is no single version for this check to look up. Verify it with:
 *   npm view @pie-players/pie-preloaded-player@<version> dist.attestations
 *
 * Usage:
 *   node scripts/check-provenance.mjs 0.2.0
...
if (!existsSync(rootManifestPath)) {
	console.error(
		"[check-provenance] run from the repository root (package.json not found).",
	);
	process.exit(1);
}
...
console.log(
	`[check-provenance] checking ${packages.length} package(s) at ${version}\n`,
);
...
for (const { name } of packages) {
	...
	if (entry.dist?.attestations) {
		console.log(`  ${name.padEnd(52)} provenance ok`);
		withProvenance++;
	} else {
		noProvenance.push([name, "published without attestations"]);
		console.log(`  ${name.padEnd(52)} NO PROVENANCE`);
	}
}
...
if (noProvenance.length > 0) {
	...
	console.log(
		"  -> likely no trusted publisher configured, or published via a token.",
	);
	console.log(
		"     Check with: bun run trusted-publishers -- --verify --only <pkg>",
	);
}
...
if (notPublished.length > 0) {
	...
	console.log(
		"  -> partial release; versioning is fixed, so all packages should move together.",
	);
}
